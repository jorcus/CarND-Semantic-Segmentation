...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    vgg_layer4 = conv_1x1(vgg_layer4_out, num_classes)
    vgg_layer3 = conv_1x1(vgg_layer3_out, num_classes)

    fcn_decoder_layer1 = upsample(vgg_layer7, kernel_size=4, strides=(2, 2))
    fcn_decoder_layer2 = tf.add(fcn_decoder_layer1, vgg_layer4)
    fcn_decoder_layer3 = upsample(fcn_decoder_layer2, kernel_size=4, strides=(2, 2))
    fcn_decoder_layer4 = tf.add(fcn_decoder_layer3, vgg_layer3)
    
    return upsample(fcn_decoder_layer4, kernel_size=14, strides=(8, 8))
    ...
    for i in range(epochs):     
        ...
        for X_train, y_train in get_batches_fn(batch_size):
            ...
            loss, _ = sess.run([cross_entropy_loss, train_op], 
                feed_dict={input_image: X_train, correct_label: y_train, keep_prob: 0.8})
            training_loss.append(loss)
            training_sample.append(len(X_train))

        training_loss = sum(training_loss)/sum(training_sample)
